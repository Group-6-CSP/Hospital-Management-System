...
function Login() {
  ...
  // ✅ Separate login logic (with patient ID fetch)
  const loginUser = async (credentials) => {
    ...
    // ✅ Critical: Fetch patientId for Patient role
    if (response.data.role === "Patient") {
      try {
        const patientRes = await axios.get(`${API_BASE}/api/patients`, {
        ...
        if (patientRes.data && patientRes.data.length > 0) {
          localStorage.setItem("patientId", patientRes.data[0].patientId);
        }
      } catch (err) {
